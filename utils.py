...
def get_data_package(args):
    ...
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset_total, batch_size=args.batch_size, shuffle=False, num_workers=0,
    )
    ...
    if not args.test:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset_total, batch_size=args.batch_size, shuffle=True, num_workers=0,
        )
...
#Lấy bảng ký tự từ nhãn 
def get_alphabet(args, type):
    global alp2num_character, alphabet_character, alp2num_radical, alphabet_radical, char2radicals
    if alp2num_character == None:
        alphabet_character_file = open(args.alpha_path,encoding="utf-8")
        ...
        if args.radical:
            alphabet_radical_file = open(args.alpha_path_radical, encoding="utf-8")
            ...
            files = open(args.decompose_path, encoding="utf-8").readlines()
            c2r = {}
            for line in files:
                items = line.strip('\n').strip().split(':')
