...
def test(epoch):
    ...
    global test_time
    test_time += 1
    torch.save(model.state_dict(), 'D:\env\history\{}/model.pth'.format(args.exp_name))
    result_file = open('D:\env\history\{}/result_file_test_{}.txt'.format(args.exp_name, test_time), 'w+', encoding='utf-8')
    ...
    for iteration in range(test_loader_len):
        data = next(dataloader) #changed
        image, label, _ = data
        #print(image.shape) #torch.Size([1, 3, 32, 256])
    ...
    if correct/total > best_acc:
        ...
        torch.save(model.state_dict(), 'D:\env\history/{}/best_model.pth'.format(args.exp_name))

    f = open('D:\env\history/{}/record.txt'.format(args.exp_name),'a+',encoding='utf-8')
    f.write("Epoch : {} | ACC : {}\n".format(epoch, correct/total))
    f.close()

if __name__ == '__main__':
    print('-------------')
    path = "D:\env\history\{}".format(args.exp_name)
    path = path.replace(os.sep, '/')
    if not os.path.isdir(path):
        os.mkdir("D:\env\history")
        os.mkdir(path)
    if args.test:
        test(-1)
        exit(0)
    ...
    for epoch in range(args.epoch):
        ...
        print(type(dataloader)) # '_MultiProcessingDataLoaderIter'
